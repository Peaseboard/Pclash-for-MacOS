...
class MihomoManager {
  ...
  Future<String?> _getBundledBinary() async {
    try {
      ...
      if (Platform.isMacOS) {
        ...
        binaryName = 'mihomo-android-arm64';
      } else {
        return null;
      }
  ...
  Future<void> stop() async {
    if (_process != null) {
      try {
        ...
        if (_api != null) {
          try {
            await _api!.updateConfig({'mode': 'direct'});
          } catch (_) {}
        }
  ...
  /// Restart with new subscription
  Future<bool> restartWithSubscription(String subscriptionContent) async {
    await stop();
    return start(subscriptionContent: subscriptionContent);
  }
